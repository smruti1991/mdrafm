...
if(isset($_POST['submit']) && $_POST['submit']=="Save" ){
   ...
   $title = $_POST['title'];
  // echo 123;exit;
   $filename = strtolower(basename($_FILES['t_calendar']['name']));
   $ext = substr($filename, strrpos($filename, '.') + 1);
   //echo  $filename;
   $md_referenceno= gen_uuid();
   $ext=".".$ext;
   $new_filename = '../pdf/'. $md_referenceno . $ext;
   $t_calendar_file = $md_referenceno . $ext;
   
   if(move_uploaded_file($_FILES['t_calendar']['tmp_name'],$new_filename)){
       
    $calendar_sql = "INSERT INTO `tranning_calendar` (title,details) VALUES ('$title', '$t_calendar_file')";
    $calendar_sql_res = mysqli_query($db,$calendar_sql);
    if($calendar_sql_res)
    {
       echo "Success";
       header("Location: tranning_calendar.php");
       exit;
    }
   }
   else{
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="title">Tranning Calendar</h5>
            </div>
            <div class="card-body">
                <?php 
                   //print_r($_POST);
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                          <label for="photo"><strong>Details</strong></label>
                          <input type="file" class="form-control-file" name="t_calendar">
                        </div>
                        </div>
                    </div>
