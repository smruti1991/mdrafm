...
if (isset($_POST["action"])) {
    if ($_POST["action"] == 'fetch') {
        ...
        $main_query = "
         ...
         SELECT  m.id,m.program_id,m.exam_title,m.total_question,m.set_question_paper,m.trng_type, f.name as faculty_name, p.prg_name as program_name, 
          pm.paper_code,m.exam_date_time,m.exam_duration,m.status,m.exam_code,m.trainee_attandance FROM `tbl_exam_master` m 
         JOIN `tbl_faculty_master` f ON m.examiner_id = f.id
         JOIN `tbl_mid_program_master` p ON m.program_id = p.id
         JOIN `tbl_mid_paper_master` pm ON m.paper_id = pm.id 
...
?>
        <table class="table table-bordered" id="trainne_attn_table" width="100%" cellspacing="0">
            ...
            <tbody>
                ...
                if($trng_type==3){
                    $object->query = "
                    SELECT e.*,CONCAT(i.name)as trainee_name,i.phone as mobile,e.attandance FROM `tbl_trainee_exam_info` e 
